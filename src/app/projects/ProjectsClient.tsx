...
export default function ProjectsClient({ projects }) {
  ...
  useEffect(() => {
    // Lazy load GSAP only on the client
      ...
      <section className="hero-section relative min-h-[60vh] flex items-center justify-center text-center bg-[#020618]">
        ...
        <div className="relative z-10 container mx-auto px-4">
          ...
          <div className="projects-count inline-flex items-center gap-2 px-6 py-3 bg-[#ededed]/10 backdrop-blur-sm rounded-full border border-[#ededed]/20">
            ...
            <span className="text-sm font-medium text-[#ededed]">{projects.length} Projects Available</span>
          </div>
        </div>
      </section>
  ...
  useEffect(() => {
    const loadGSAPForCard = async () => {
      try {
        ...
        console.warn('GSAP failed to load for card:', error);
        // Continue without animations if GSAP fails
      }
    };
